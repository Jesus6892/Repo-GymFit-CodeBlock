...
int InscripcionesArchivo::buscar(int idAlumno, int idHorario) const {
    FILE* pInsc;
    Inscripciones reg;
    int pos = 0;
    pInsc = fopen(_ruta.c_str(), "rb");
    if (pInsc == nullptr) {
        ...
        return -2; // Indica que hubo un error al abrir el archivo
    }

    // Recorrer cada registro en el archivo
    while (fread(&reg, _tamReg, 1, pInsc) == 1) {
        // Comparar ambos ID: idAlumno e idHorario
        if (reg.getIdAlumno() == idAlumno && reg.getIdHorario() == idHorario) {
            fclose(pInsc);
            return pos; // Retorna la posición si ambos ID coinciden
        }
        pos++;
    }

    fclose(pInsc);
    return -1; // No se encontró la inscripción con ambos ID
}
